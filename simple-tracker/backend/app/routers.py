...
async def api_info():
    return {
        "message": f"API for the live monitoring and analyzing. Go to /docs for more info. "
    }


@router.get("/api/live")
async def get_live_poz(
    db: Session = Depends(get_db), token: str = Depends(sec.oauth2_scheme)
):
...
@router.post("/token")
async def login(
    db: Session = Depends(get_db), form_data: sec.OAuth2PasswordRequestForm = Depends()
):
    ...
    check_credentials = crud.check_hash_password_is_in_db(
        db, sec.get_password_hash(form_data.username, form_data.password)
    )
...
async def create_user(user: schemas.UserSchema, db: Session = Depends(get_db)):
    user_exist = crud.check_if_creating_user_exist(
        db, user.email, sec.get_password_hash(user.email, user.password)
    )
    if user_exist == True:
        raise HTTPException(status_code=409, detail="Confilct")
    return crud.create_user(
        db, details=user, hash_password=sec.get_password_hash(user.email, user.password)
    )
