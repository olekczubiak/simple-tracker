...
def get_owner_id_by_token(db: Session, my_token: str):
    return (
        ...
        .filter(models.User.hashed_password == my_token)
        .first()[0]
    )
...
def get_list_poz_by_day(db: Session, user_id: int, my_day: str):
    return (
        db.query(models.Position)
        .filter(models.Position.owner_id == user_id)
        .filter(models.Position.my_date == my_day)
        .all()
    )
...
def get_list_of_events_in_month(db: Session, user_id: int, my_month: int):
    ...
    for day in range(1, 31):
        month = (
            db.query(models.Position)
            .filter(models.Position.owner_id == user_id)
            .filter(extract("month", models.Position.my_date) == my_month)
            .filter(extract("day", models.Position.my_date) == day)
            .filter(models.Position.time == daily_separator)
            .all()
        )
        if len(month) > 0:
...
def check_hash_password_is_in_db(db: Session, hash_pass: str):
    return (
        ...
        .filter(models.User.hashed_password == hash_pass)
        .first()
    )
